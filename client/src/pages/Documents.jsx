  ...
  if (!recipient) {
    return (
      <div className="p-6 flex flex-col items-center justify-center min-h-[60vh] text-center">
        <FileText size={40} className="text-mist mb-4" />
        <h2 className="text-lg font-semibold text-night mb-2">No care circle yet</h2>
        <p className="text-mist text-sm">Set up a circle first to start storing documents.</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 lg:p-6 max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-night">Documents</h1>
      ...
      {showForm && (
        <div className="bg-white rounded-card shadow-card p-5">
          <h2 className="text-sm font-semibold text-night mb-4">Upload a document</h2>
          <form onSubmit={handleUpload} className="space-y-3">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-colors ${
                file ? 'border-sage bg-sage/5' : 'border-cloud hover:border-sage/50 hover:bg-dawn'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              {file ? (
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="text-sm font-medium text-night truncate max-w-[200px]">{file.name}</span>
                  ...
                  <button
                    ...
                    className="text-mist hover:text-rose ml-1"
                  >
                    <X size={14} />
                  </button>
                </div>
                ...
                <>
                  <Upload size={24} className="text-mist mx-auto mb-2" />
                  <p className="text-sm text-mist">Tap to select a photo or PDF</p>
                  <p className="text-xs text-mist mt-0.5">JPEG, PNG, WebP, HEIC, PDF — up to 20 MB</p>
                </>
              )}
            </div>

            <div>
              <label className="block text-xs font-medium text-mist mb-1.5 uppercase tracking-wide">Label</label>
              <select
                value={label}
      ...
      <section>
        ...
        {loading ? (
          <div className="space-y-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-16 bg-white rounded-card shadow-card animate-pulse" />
            ))}
          </div>
        ) : documents.length === 0 ? (
          <div className="bg-white rounded-card shadow-card p-10 text-center">
            <FileText size={32} className="text-mist mx-auto mb-3" />
            <p className="text-sm font-medium text-night mb-1">No documents yet</p>
            <p className="text-xs text-mist">Upload medical records, lab results, prescriptions, and more.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {documents.map((doc) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-night truncate">{doc.file_name}</p>
                  <div className="flex items-center gap-2 flex-wrap mt-0.5">
                    ...
                    {doc.uploader?.full_name && (
                      <span className="text-xs text-mist">· {doc.uploader.full_name}</span>
                    )}
                    {doc.file_size && (
                      <span className="text-xs text-mist">· {formatBytes(doc.file_size)}</span>
                    )}
                  </div>
                  {doc.notes && (
                    <p className="text-xs text-mist mt-0.5 truncate">{doc.notes}</p>
                  )}
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  {doc.url && (
                    <a
                      ...
                      className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-dawn text-mist hover:text-night transition-colors"
                      aria-label="Download"
                    >
                      <Download size={15} />
                    </a>
                  ...
                  <button
                    ...
                    className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-rose/10 text-mist hover:text-rose transition-colors"
